...
for i in range(len(contracciones['contraccion'])):
    tweets.loc[0].replace(to_replace=contracciones['contraccion'][i], value = contracciones['palabra'][i], regex=True, inplace=True)
    
#Reemplazando abreviaciones
for i in range(len(abreviaciones['abreviacion'])):
    tweets.loc[0].replace(to_replace=abreviaciones['abreviacion'][i], value = abreviaciones['palabra'][i], regex=True, inplace=True)
